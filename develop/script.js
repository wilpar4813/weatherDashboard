  ...
  // // })
     ...
     // .then(function(response) {
       ...
       // console.log(response);
        ...
        $(".city").html("<h1>" + response.name + " (" + startDate.format('L') + ") " + response.weather.icon + "</h1>");
...
// Transfer content to HTML
    ...
    function getUV(){
          // Here we are building the URL we need to query the UV database
         
      var queryURL = "https://api.openweathermap.org/data/2.5/uvi?" +
      "lat=37.75&lon=-122.37" + APIKey;
    // Here we run our AJAX call to the OpenWeatherMap API
